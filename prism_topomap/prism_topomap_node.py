...
class ResultsPublisher(Node):
    ...
    def publish_localization_results(self, graph, vertex_ids_matched, rel_poses, vertex_ids_unmatched):
        if len(vertex_ids_matched) > 0:
            ...
            vertices_marker.points.append(Point(x=x, y=y, z=0.1))
            ...
            vertices_marker.id = 0
            vertices_marker.header.frame_id = self.map_frame
            vertices_marker.header.stamp = self.current_stamp
            ...
            transforms_marker.header.stamp = self.current_stamp
            transforms_marker.scale.x = 0.1
            transforms_marker.color.r = 1.0
            transforms_marker.color.g = 0.0
            transforms_marker.color.b = 0.0
            transforms_marker.color.a = 0.5
            transforms_marker.pose.orientation.w = 1.0
            for vertex_dict, rel_pose in zip(localized_vertices, rel_poses):
                x, y, theta = vertex_dict['pose_for_visualization']
                transforms_marker.points.append(Point(x=x, y=y, z=0.1))
    ...
    def publish_ref_cloud(self, cloud, stamp):
        ...
        cloud_msg = ros2_numpy.point_cloud2.dict_to_point_cloud2(cloud_with_fields)
        if stamp is not None:
            cloud_msg.header.stamp = stamp
        else:
            cloud_msg.header.stamp = self.current_stamp
        cloud_msg.header.frame_id = 'base_link'
        self.ref_cloud_pub.publish(cloud_msg)

    def publish_gt_map(self):
        print('Publish gt map')
        gt_map_msg = OccupancyGrid()
        gt_map_msg.header.stamp = self.current_stamp
        ...
        gt_map_msg.info.origin.orientation.x = 0.0
        gt_map_msg.info.origin.orientation.y = 0.0
        gt_map_msg.info.origin.orientation.z = 0.0
        gt_map_msg.info.origin.orientation.w = 1.0
    ...
    def publish_graph(self, graph):
        ...
        for vertex_dict in graph.vertices:
            x, y, _ = vertex_dict['pose_for_visualization']
            vertices_marker.points.append(Point(x=x, y=y, z=0.05))
        ...
        for u in range(len(graph.vertices)):
            for v, pose in graph.adj_lists[u]:
                ...
                edges_marker.points.append(Point(x=ux, y=uy, z=0.05))
                edges_marker.points.append(Point(x=vx, y=vy, z=0.05))
        ...
        vertex_orientation_marker.color.r = 1.0
        vertex_orientation_marker.color.g = 0.0
        vertex_orientation_marker.color.b = 0.0
        vertex_orientation_marker.color.a = 1.0
        vertex_orientation_marker.pose.orientation.w = 1.0
        for vertex_dict in graph.vertices:
            x, y, theta = vertex_dict['pose_for_visualization']
            vertex_orientation_marker.points.append(Point(x=x, y=y, z=0.1))
            vertex_orientation_marker.points.append(Point(x=x + np.cos(theta) * 1, y=y + np.sin(theta) * 1, z=0.05))
    ...
    def publish_last_vertex(self, last_vertex, last_vertex_id):
        marker_msg = Marker()
        marker_msg.header.stamp = self.current_stamp
        ...
        tf_msg.header = marker_msg.header
        tf_msg.child_frame_id = 'vcur'
    ...
    def publish_loop_closure_results(self, graph, path, global_pose_for_visualization):
        ...
        vertices_marker.points.append(Point(x=ux, y=uy, z=0.05))
        vertices_marker.points.append(Point(x=vx, y=vy, z=0.05))
        vertices_marker.points.append(Point(x=x, y=y, z=0.05))
        ...
        edges_marker.header.stamp = self.current_stamp
        ...
        ux, uy, _ = graph.get_vertex(u)['pose_for_visualization']
        edges_marker.points.append(Point(x=ux, y=uy, z=0.05))
        edges_marker.points.append(Point(x=x, y=y, z=0.05))
        vx, vy, _ = graph.get_vertex(v)['pose_for_visualization']
        edges_marker.points.append(Point(x=vx, y=vy, z=0.05))
        edges_marker.points.append(Point(x=x, y=y, z=0.05))
    ...
    def publish_local_grid(self, local_grid):
        ...
        local_grid_msg.info.origin.orientation.x = 0.0
        local_grid_msg.info.origin.orientation.y = 0.0
        local_grid_msg.info.origin.orientation.z = 0.0
        ...
        local_grid_msg.data = local_map.flatten().tolist()
    ...
    def publish_cur_grid(self, cur_grid):
        ...
        np.savetxt('/home/docker_prism/local_map.txt', local_map)
        #local_map[local_map == 0] = -1
        #local_map[local_map == 1] = 0
        local_grid_msg.data = local_map.flatten().tolist()
    ...
    def publish_rel_pose(self, rel_pose_of_vcur):
        ...
        rel_pose_msg.header.stamp = self.current_stamp
        ...
        rel_pose_msg.pose.position.z = 0.0
        ...
        tf_msg = TransformStamped()
        tf_msg.header.stamp = rel_pose_msg.header.stamp
        tf_msg.header.frame_id = 'vcur'
        tf_msg.child_frame_id = 'current_state'
    ...
    def publish_topological_path(self, graph, path):
        ...
        for v_id in path:
            ...
            pt = Point(x=pos[0], y=pos[1], z=0.2)
    ...
    def publish_subgoal(self, subgoal):
        x, y, theta = subgoal
        tf_msg = TransformStamped()
        tf_msg.header.stamp = self.current_stamp()
        tf_msg.header.frame_id = 'current_state'
        tf_msg.child_frame_id = 'next_state'
        ...
        pointgoal_msg.pose.position.z = 1.0
...
class PRISMTopomapNode(Node):
    ...
    def init_publishers_and_subscribers(self, config):
        # Visualization
        self.get_logger().info('Initializing publishers and subscribers')
        ...
        # parallel_callback_group = MutuallyExclusiveCallbackGroup()
        # self.parallel_callback_group = parallel_callback_group
    ...
    def odom_callback(self, msg):
        ...
        msg_time = float(msg.header.stamp.sec) + float(msg.header.stamp.nanosec)/1e9
        dt = current_time - msg_time
        # self.get_logger().info('Odometry oldness: {}'.format(dt))
        x, y = msg.pose.pose.position.x, msg.pose.pose.position.y
        orientation = msg.pose.pose.orientation
        _, __, theta = tf.euler_from_quaternion(
            [orientation.x, orientation.y, orientation.z, orientation.w]
    ...
    def pcd_callback(self, msg):
        ...
        current_time = self.get_clock().now().nanoseconds / 1e9
        msg_time = float(msg.header.stamp.sec) + float(msg.header.stamp.nanosec)/1e9
        # self.get_logger().info('Point cloud of time {} has received'.format(msg_time))
        dt = current_time - msg_time
        # self.get_logger().info('Point cloud oldness: {}'.format(dt))
        if dt > 10000:
        ...
        if dt > 0.5 or dt < -0.5:
            self.get_logger().warning('Skip message with big time difference ({} seconds)'.format(dt))
            return
        # if len(self.odom_poses) > 0:
        #     last_odom_time = self.odom_poses[-1][0]
        #     self.get_logger().info('Odom - pcd dt: {}'.format(last_odom_time - msg_time))
